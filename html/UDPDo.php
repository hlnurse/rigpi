...
require_once "/var/www/html/classes/UDPServer.class.php";
...
//echo $tMyIP;
$UDPServer = new UDPServer($tMyIP, $tMyPort, $tMyRadio, $tUser);
...
$UDPServer->infinite_loop(); // Run Server Code Until Process is terminated.
...
function handle_input(&$server, &$client, $input, $radio, $user)
{
  ...
  //$user='admin';
  //$radio=1;
  $tWLogOK = getUserField($user, "LogWSJTX");
  if ($tWLogOK == 0) {
    return;
  }
  $tLogName = GetField($radio, "LogName", "MySettings");
  if ($tLogName == "ALL Logs" || $tLogName == "") {
    $tLogName = "WSJTX";
  }
  //	$input="<call:5>K7XYZ <gridsquare:4>DM42 <mode:3>FT8 <rst_sent:3>-12 <rst_rcvd:3>-08 <qso_date:8>20201106 <time_on:6>005730 <qso_date_off:8>20201106 <time_off:6>005830 <band:3>20m <freq:9>14.076341 <station_callsign:4>W6HN <my_gridsquare:4>CM87 <eor>";
  // echo $input;
  //	$input="<call:5>K7CTV <gridsquare:4>DM42 <mode:3>MFSK <submode:3>FT4 <rst_sent:3>-12 <rst_rcvd:3>-20 <qso_date:8>20201106 <time_on:6>010653 <qso_date_off:8>20201106 <time_off:6>010653 <band:3>20m <freq:9>14.082341 <station_callsign:4>W6HN <my_gridsquare:4>CM87 <eor>";
  $trim = trim($input);
  ...
  $comms1 = $trim;
  /*  if (strstr($comms1, H0C) > 0) {
    $tADIFOn = 1;
  } elseif (strstr($comms1, "EOF")) {
    $tADIFOff = 1;
  }
*/
