...
if ($port > 4500 && $port < 5000) {
  ...
  if ($db->count > 0) {
    $model=$row["Model"];
    if (strtolower($row["Port"]) != "none" && $model !=="Net rigctl") {
      echo $row["Port"] .
...
*/
  ...
  if ($db->count > 0) {
    if (strtolower($row["SlavePort"]) != "none") {
      echo $row["SlavePort"] .
        " is already is also in use as a slave port in the account for radio " .
        $row["RadioName"] .
        ".";
//      exit();
    }
