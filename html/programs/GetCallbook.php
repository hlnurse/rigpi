...
if ($getWhat == "QRZData") {
  if ($row && strlen($row["Callsign"]) > 0) {
    ...
    $data .=
      "<br><br>" .
      "$countyData" .
      "Latitude: $dxLat<br>Longitude: $dxLon<br>Distance: $dMi mi, $dKM km<br>Bearing: $dBearing Deg<br>Grid: $grid<br>DXCC: $entity Number: $dxcc<br>CQ Zone: $cqZ<br>ITU Zone: $ituZ<br>WPX: $wpx<br>Source: $source";
    echo $data;
  } else {
    if ($call == "NG") {
      $call = "Call";
  ...
  if ($row && strlen($row["Callsign"]) > 0) {
    ...
    $data .= "DXCC: $entity ($dxcc)<br>CQ Zone: $cqZ&nbsp;&nbsp;&nbsp;<br>ITU Zone: $ituZ<br>WPX: $wpx<br>Source: $dataS";
    echo $data;
  } else {
    if ($call == "NG") {
      $call = "Call";
