  ...
  while($row1 = $result4->fetch_assoc()){
    if (!is_null($row1)){
      ...
      if ($aD==1){
        ...
        $tEcho4 = $tEcho4 . "<div style='color: yellow'>Upcoming reservation for $callsign with $rname in " . intval($dif) . " $t</div>";
      }
    }else{
      $tEcho4=$tEcho4 . "<div style='color: white'>All radios are available now</div>";
    }
  }
}else{
    $tEcho4=$tEcho4 . "<div style='color: white'>All radios are available xxxnow</div>";
}
