...
//		$tTable=$tTable."<th class='hClk' style='text-align:center; vertical-align:middle'><input id='sel' name='selAll' type='checkbox'>";
...
while ($i< $db->count){
	...
	while ($j< $dbStyle->count){
		$tRowStyle=$styles[$j];
		if ($tRowStyle['IDValue']=='Sel'){
			$tTable.="<td class='selCk' style='text-align: center;'><input id='c".$tID."' type='checkbox' name='Sel' " . $tVal . "></td>";
		}else{
			$val='';
			if ($tRowStyle['Attribute']=='Add Periods' && $tRow[$tRowStyle['Field']]>0){
				...
				if ($tRowStyle['IDValue']=='Note'){
					$val=$tRow[$tRowStyle['Field']];
					$tTable=$tTable."<td class='Note' style='width:10%'>" . $val. "</td>";
