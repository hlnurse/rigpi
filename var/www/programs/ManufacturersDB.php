...
function getRadioManufacturers(){
	...
	if ($result->num_rows > 0) {
		while ($row = $result->fetch_assoc()){
			...
			$data=$data . "<div class='myman' id='$man'><li><a class='dropdown-item manufacturers' id='$man' href='#'>" . $man . "</a></li></div>\n\r";
		}
	}
	return $data;
}

function getRotorManufacturers(){
	$sql_radio_username="";
	$sql_radio_password="";
	$sql_radio_database="";

	require "/var/www/html/programs/sqldata.php";
	$str2 = "SELECT DISTINCT MANUFACTURER from Rotors ORDER BY MANUFACTURER";
	$con = new mysqli("localhost", $sql_radio_username, $sql_radio_password, $sql_radio_database);
	if ($con->connect_error) {
		die("Connection failed: " . $con->connect_error);
	}
