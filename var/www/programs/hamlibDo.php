...
$report .= "RigDo.php rigDoPID from settings: " . $tRigDoPID . "<br>";
if ($test == 1) {
  echo $report;
}
if ($id == 2) {
...
//	if (strlen($pos>0)){
disRadio($tMyRadio, $tUsername, $tMyRotor);
//	}
if ($id == "1" || $id == "2") {
  $execDum = "sudo rigctld -m $id -t $service_port $ptt $vfoParam > /dev/null 2>/dev/null &";
  //		$report.=$result;
  $r = "";
  if ($id == "1") {
    $r = "Hamlib Dummy";
  ...
  if ($test == 1) {
    ...
    if ($id == 2) {
      $rd = shell_exec(
        "ps aux | grep [r]igctl -m $id -r localhost:$service_port"
      );
    } else {
  ...
  $execMe = "sudo rigctld -m $id -r $port -t $service_port $ptt $baud $tCIV $stop $tDTR $tRTS $vfoParam $setConf > /dev/null 2>/dev/null &";
  ...
  $pr = "";
  while (intval($rd) < 1) {
    //    $pr = shell_exec("ps aux | grep [r]igctl -m 2 -r 127.0.0.1:$service_port");
    //    echo "i2: " . $i . " rd: " . $rd . "\n";
    usleep(1000000);
    $rd = shell_exec($gF);
    $i += 1;
...
if (!$socket) {
  ...
  doLog($test, "Socket_create succeeded.");
  doLog($test, "Last socket error: " . socket_strerror(socket_last_error()));
  $sc = socket_connect($socket, $address, $service_port);
  if ($sc == true) {
    $tsc = "true";
  } else {
    $sc = socket_connect($socket, $address, $service_port);
    if ($sc == true) {
      ...
      doLog(
        $test,
        "Socket_connect failed (1): reason: " .
          socket_strerror(socket_last_error())
      );
    }
  }
  doLog(
    $test,
    "socket connect returned $tsc, $socket, $address, $service_port."
  );
  if ($vfoMode == 1) {
    $in = "f " . $vfoa . "\n";
  } else {
  ...
  $in = str_replace($in, "\n", "");
  $u = "socket write to radio " . $tMyRadio . ": $in returned " . $tsw;
  doLog($test, $u);
  $i = 0;
  while ($tsw == "false") {
    ...
    doLog(
      $test,
      "socket write to radio " . $tMyRadio . " attempt $i: f returned $tsw"
    );
    usleep(1000000);
    $i += 1;
    if ($i == 40) {
      echo "\nSocket connection failed (try reboot and check Baud rate).\n";
      doLog($test, "Socket failed to connect after 20 attempts.");
      exit();
    }
  }
  $reportOut = "";
  ...
  while ($OK) {
    ...
    $reportOut .= $currentByte;
  }
  $reportOut = str_replace("\n", "", $reportOut);
  doLog($test, "Response from radio (first attempt): " . $reportOut);

  if (strpos($reportOut, "RPRT") !== false) {
    socket_write($socket, $in, strlen($in));
    $reportOut = substr($reportOut, 0, strlen($reportOut) - 1);
    $reportOut = "";
    $OK = true;
    while ($OK) {
      $currentByte = socket_read($socket, 1);
    ...
    doLog($test, "Response from radio (second attempt): " . $reportOut);
    $report =
      "Error connecting to radio (" .
      $reportOut .
      ")(0). Check radio connection and settings. Is the radio powered up? Please try again.<br>";
    doLog($test, $report . PHP_EOL . " " . $reportOut);
    if ($test == 1) {
      echo $report;
    }
    exit();
...
*/
/*
if (strstr($reportOut, "RPRT")) {
  echo "Hamlib error: " .
    strlen($report) .
    "<br>" .
    "Rigctld timed out waiting for $dmodel.<br>";
  doLog(
    $test,
    "Hamlib error: " .
      strlen($report) .
      ". Rigctld timed out waiting for $dmodel."
  );
} elseif (strstr($report, "failed")) {
  if ($test == 1) {
    if (strlen($reportOut) == 0) {
      $reportOut = "no failure report";
      doLog($test, "Hamlib error: null failure report.");
    }
    echo "Startup of $dmodel failed: " . $report . "<br>";
    doLog($test, "Startup of $dmodel failed: " . $report . ".");
  }
  echo "Connection failed (1), check radio port settings.  Is the radio on?<br>";
  doLog(
    $test,
...
if ($test == 1) {
  if (strlen($curFreq) > 5 && intval($curFreq) > 1000) {
    ...
    $report .=
      "Main frequency is " .
      (number_format($curFreq, 0, "", ".") . " MHz<br><br>");
    doLog(
      $test,
...
if ($rotorID == "1" || $rotorID == "2") {
  ...
  if ($test == 1) {
    $report .=
      "To see any error details, click Disconnect Radio, then start rotctld in Terminal with this line: <br><br><b>" .
      $execDum .
      "</b><br>";
    echo $report;
