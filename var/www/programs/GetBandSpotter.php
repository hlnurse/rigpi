...
while ($i< $tRowCount){
	...
	$top=(-15+(($tUp-$frequency)/100)); //-15px is half height of button
	$ftop=$top;
	$top1=$pTop-$top;
	$spottertext=$tText."\nMins: ".number_format((time()-$tRow['Webdate'])/60,2,".",",")."\nDE: ".$spotter;
	if ($top1<=30){
		$top=$top-38+$top1;
	} 
	$pTop=$top;
	$top=$top.'px';
	$spots.="<button ".$tColor." id='b".$tID."' call='$DX' style='top: $top;' ftop='$ftop' band='$tBand' frequency='$frequency' mode='$tMode' title='$spottertext' >$DX</button><br>";
	$i++;
}
