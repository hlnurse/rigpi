...
function getModes($tMyRadio){
	...
	if (strlen($mode)>0){
		...
		foreach ($list as $mode){
			$data=$data . "<div class='mymode'><li><a class='dropdown-item' id='m0' href='#'>" . $mode . "</a></li></div>\n";
		}
	}else{
		$db->orderBy('Name','ASC');
		$data="";
		...
		foreach($modes as $mode){
			$data=$data . "<div class='mymode'><li><a class='dropdown-item' id='m0' href='#'>" . $mode['name'] . "</a></li></div>\n";
			$i=$i+1;
		}
	}
	return $data;
