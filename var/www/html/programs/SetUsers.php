 ...
 */
	...
	$Fl=$_POST['LogFldigi'];
	$WS=$_POST['LogWSJTX'];
	if ($Fl==1){  //clears all log fields since only one instance can sync logs
		$tD=Array (
			'LogFldigi' => 0
			);
			$db->update ('Users', $tD);
	...
	if ($WS==1){  //clears all log fields since only one instance can sync logs
		$tD=Array (
			'LogWSJTX' => 0
			);
			$db->update ('Users', $tD);
	...
	if (strlen($tPWD)>0) {
		...
		if ($refID>0){
			...
			$db->where("uID",$refID);
			if ($db->update ('Users', $data)){echo "<br>Username ".$_POST['Username']." updated.<p>";}else{echo "<br>Username must be unique!<p><p>Changes not saved.<p>";};
		}else{
			$row = $db->getOne("Users");
			$data = Array (
				'uID'=>$_POST['ID'],
			...
			if ($db->insert ('Users', $data)){echo "<br>Username ".$_POST['Username']." added to Users.<p>";}else{echo "<br>Username must be unique!<p><p>Changes not saved.<p>";};
		}
	}else{
		if ($refID>0){
			$data = Array (
