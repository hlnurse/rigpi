...
{
  //  echo "incoming SPEED: " . $input . "\n";
  // if (strpos($input, "KS") !== false) {
  $comm = substr($input, 2, 1);
  //  echo "now SPEED: " . intval(bin2hex($comm)) . " len: " . strlen($comm) . "\n";
  //$comm = str_replace(";", "", $comm);
  //  echo "Keyer speed: " . hexdec(bin2hex($comm)) . "\n";
  $commI = ord($comm);
  $comm1 = 0x3f & hexdec(bin2hex($comm));
  //  echo "after 80 xxxKeyer speed: len: " .
  strlen($comm1) .
    " " .
    $comm1 .
    " lenhexdec: " .
    strlen(hexdec(bin2hex($comm))) .
    "\n";
  SetKeyerSpeed($radio, "WKSpeed", $comm1);
  $tData = "1";
  SetInterface($radio, "CWChangeCk", $tData);
  for ($i = 0; $i < count($client); $i++) {
    $server->socket_write_smart($client[$i]->socket, "KS" . $comm . ";", "");
  }
  //  }
}
...
function handle_input(&$server, &$client, $input, $call, $radio)
{
  ...
  foreach ($comms as $comm) {
    if (strlen($comm) > 0) {
      ...
      switch (true) {
        case strstr($comm, "KY"):
          //         echo "KY found: " . bin2hex($comm) . " " . $client . "\n";
          $comm = str_replace(";", "", $comm);
          $comm = str_replace("KY", "", $comm);
          //          echo "comm: " . bin2hex($comm) . "\n";
          /*         if (strstr($comm, "@")) {
  $comm = str_replace("@", chr(0x1b) . "AR", $comm);
}
if (strstr($comm, "_")) {
  $comm = str_replace("_", chr(0x1b) . "BT", $comm);
}
if (strstr($comm, "#")) {
  $comm = str_replace("#", chr(0x1b) . "KN", $comm);
}
if (strstr($comm, "'A")) {
  $comm = str_replace("'A", chr(0x1b) . "AR", $comm);
}
if (strstr($comm, "'K")) {
  $comm = str_replace("'K", chr(0x1b) . "KN", $comm);
}
if (strstr($comm, "'B")) {
  $comm = str_replace("'B", chr(0x1b) . "BK", $comm);
}
if (strstr($comm, "'C")) {
  $comm = str_replace("'C", chr(0x1b) . "CL", $comm);
}
if (strstr($comm, "'S")) {
  $comm = str_replace("'S", chr(0x1b) . "SK", $comm);
}
if (strstr($comm, "'Z")) {
  $comm = str_replace("'Z", chr(0x1b) . "AS", $comm);
}
if (strstr($comm, ">")) {
  $comm = str_replace(">", chr(0x1c) . chr("35"), $comm);
}
if (strstr($comm, "<")) {
  $comm = str_replace("<", chr(0x1e), $comm);
}
*/
