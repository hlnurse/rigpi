...
class RigServer
{
  ...
  public function __construct($bind_ip, $port, $radio, $test, $vfo, $vfoMode)
  {
    ...
    $defAudio = trim(shell_exec("grep -m 1 'card' /home/pi/.asoundrc"));
    $this->config["audioCard"] = trim(substr($defAudio, 5));
    $defAudioType = "CODEC"; //shell_exec("sudo aplay -l | grep -m 1 '".$defAudio."'");
    //			$this->config['test']="def: ".$defAudioType;
    if (strstr($defAudioType, "CODEC")) {
      $this->config["audioType"] = "C";
    } else {
      $this->config["audioType"] = "A";
    }
    if ($this->config["port"] !== 0) {
      ...
      /*		    	$command="l AF";
				...
				$trig=str_replace("\n", "", $trig);
				$tField="AFGain";
				$tData=intval($trig * 100);
				$this->config["AFL"]=$tData;
				$this->setRigInterface($radio,$tField,$tData);
...
*/
    ...
    ///		modprobe removes ftdi_sio to accomodate ftdi parallel adapter
    if (
  ...
  public function getRigMessage($count)
  {
    ...
    //    $tR1 = $this->rig_socket_read($this->rig_socket, $count);
    $tR = ltrim($tR, "\n"); //trims leading \n
  ...
  public function loop_once()
  {
    ...
    if ($tPTTCk == 1) {
      ...
      if ($this->config["PTTCAT"] == 1 || $this->config["PTTMode"] == 2) {
        if ($tPTT == 1) {
          $tCard = $this->config["audioCard"];
          $this->config["inTransmit"] = "on";
          if ($this->config["PTTDelay"] > 0) {
            $vol = 0;
            if ($this->config["audioType"] == "C") {
              $volpc = shell_exec(
                "amixer -c $tCard get PCM | grep 'ht: Playback '"
              );
              exec("amixer -q -c $tCard set PCM 0");
            } else {
              $volpc = shell_exec(
                "sudo amixer -c $tCard get Master | grep 'ht: Playback '"
              );
              exec("amixer -q -c $tCard set Master 0");
            }
            $volpc1 = explode(" ", $volpc);
            $volpc = $volpc1[5];
            ...
            if ($this->config["PTTDelay"] > 0) {
              if ($this->config["audioType"] == "C") {
                exec("amixer -q -M -c $tCard set PCM $volpc");
              } else {
                exec("amixer -q -M -c $tCard set Master $volpc");
              }
            }
          } else {
            $this->setRigInterface($tRadio, "Transmit", "1");
  ...
  public function loop_CheckOnce()
  {
    ...
    if ($this->config["CWDeadman"] == 0) {
      ...
      $cwState = 1; /////////////////////////////////////////////////////////////////////////////system("gpio read 13"); //cw deadman 15-20 secs w/500 below
      if ($cwState == 0) {
    ...
    if ($tCommCk == 1) {
      ...
      if ($tComm == "PS0;") {
        ...
        $bunch = explode("*", $tComm);
        for ($i = 1; $i < count($bunch); $i++) {
          if (strpos($bunch[$i], ">") !== false) {
            $bunch[$i] = str_replace(">", "", $bunch[$i]);
            $this->setRigInterface($tRadio, "CommandOut", "0");
          }
          if (strpos($bunch[$i], "w ") !== false) {
            ...
            if (
              ...
              if (strlen($trig) > 0) {
                ...
                $this->doLog("Radio return from custom command: " . $oops);
                //										$this->setRigInterface($tRadio,"RadioData",$oops);
